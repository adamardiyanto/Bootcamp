...
class Program
{
    ...
    Animal hewan = new Animal("hewan",12);
    XmlSerializer serializer = new XmlSerializer(typeof(Cat));
    using(StreamWriter writer = new StreamWriter("cat.xml"))
        {
            serializer.Serialize(writer, cat1);
            serializer.Serialize(writer, cat2);
            serializer.Serialize(writer, hewan);
        }
    }
}